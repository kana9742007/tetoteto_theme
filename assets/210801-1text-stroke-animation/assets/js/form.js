$(function() {
  ...
  function recruitingSet(){
    var selector = $('select[name=category]');
    var param = location.search;
    console.log('param:' + param);
    if(param.indexOf('recruit') != -1) {
      selector.val('採用に関するご相談');
      console.log('recruit');
    }else{
      console.log('other');
    }
  ...
  function setMyForm(target){
    ...
    function processOrderContent(){
      ...
      var company = target.find('input[name=corporatename]').val();
      var telNumber = target.find("input[name=telnumber]").val();
      var content = target.find("textarea[name=content]").val();
      event.preventDefault();
      $.ajax({
        url: "https://docs.google.com/forms/u/0/d/e/1FAIpQLScVFmfyZPgQqevi6avnFF7zdXsYhpyZw09pxbj80R9AVO8CSg/formResponse",
        data: {
          "entry.485794924": category,
          "entry.881706862": userName,
          "entry.1956508365": userMail,
          "entry.1833788608": company,
          "entry.2081206857": telNumber,
          "entry.763608966": content,
        },
      type: "POST",
      dataType: "xml",
      statusCode: {
